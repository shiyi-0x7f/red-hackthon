...
pub fn sanitize_output(text: &str) -> String {
    ...
    for word in BANNED_WORDS {
        if result.contains(word) {
            tracing::warn!("Banned word detected: {}", word);
            result = result.replace(word, "[已过滤]");
        }
    }

    // 2. 检查情感绑定句式
    if contains_emotional_binding(&result) {
        tracing::warn!("Emotional binding detected");
        return String::new();
    }
...
| 学习感受 | 考试压力 | 家庭矛盾细节 |
| 兴趣爱好 | 同学关系 | 心理疾病讨论 |
| 最近看的书/动画 | 情绪低落 | 自伤/自杀话题 |
| 运动/生活 | 学业焦虑 | 恋爱/性相关 |

### 4.2 敏感内容升级机制

```rust
pub fn handle_sensitive_content(message: &str) -> Option<SensitiveResponse> {
    if detect_sensitive_content(message) {
        Some(SensitiveResponse {
            reply: "我注意到你提到的这些，这些很重要。建议你和信任的大人聊一聊，比如家长或老师。".to_string(),
            escalate: true,
            log_event: true,
        })
    } else {
...
pub fn build_prompt(template: &str, context: &PromptContext) -> String {
    template
        .replace("{grade}", &context.grade.to_string())
        .replace("{knowledge_name}", &context.knowledge_name)
        .replace("{student_brief}", &context.student_brief)
